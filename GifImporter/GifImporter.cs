...
namespace GifImporter
{
	public class GifImporter : NeosMod
	{
		...
		public static ModConfigurationKey<bool> KEY_SQUARE = new ModConfigurationKey<bool>(
			"square_images", "Generate square tiles (sometimes has bigger size)", () => false);
		...
		{
			public static bool Prefix(string path, ref Task __result, Slot targetSlot, float3? forward, StereoLayout stereoLayout, ImageProjection projection,
				bool setupScreenshotMetadata, bool addCollider)
			{
				...
				__result = targetSlot.StartTask(async delegate ()
				{
					...
					Uri localUri = await
						localDB.ImportLocalAssetAsync(spritePath, LocalDB.ImportLocation.Copy).ConfigureAwait(continueOnCapturedContext: false);
